...
flowchart TD
    ...
    User -->|"1. POST /queue"| QueueApi
    QueueApi -->|"2. token 등록"| QueueEntryLua
    QueueEntryLua --> RedisQueue
    User -->|"3. GET /queue/{token}"| QueueApi
    QueueApi -->|"4. waiting rank / active TTL 조회"| RedisQueue
    Scheduler -->|"5. 매초 admission rate만큼 전환"| AdmissionLua
    AdmissionLua --> RedisQueue
    User -->|"6. POST /reservations"| ReservationApi
    ReservationApi --> ActiveGuard
    ActiveGuard --> RedisQueue
    ReservationApi --> ClaimLua
    ClaimLua --> RedisReservation
    ReservationApi -->|"7. RESERVED event 발행"| Stream
    Stream --> Worker
    Worker --> MySQL
```
